...
export class TailwindConfigParser {
	...
	constructor(tailwindConfig: TTailwindCSSConfig, plugins: TConfigPlugins) {
		...
		this._darkMode = _.isEmpty(tailwindConfig?.darkMode) ? false : (tailwindConfig.darkMode as TConfigDarkMode);
		this._separator = _.isEmpty(tailwindConfig.separator) ? ":" : (tailwindConfig.separator as string);
		this._themeConfig = {
	...
	public getThemeProperty = (
		...
		return [Object.keys(this.getTheme()[themeProperty]), Object.values(this.getTheme()[themeProperty])];
	};
}

...
import * as ClassnamesTypes from "../types/classes";
...
export class ClassnamesGenerator {
	...
	private readonly _generatedRegularClassnames: ClassnamesTypes.TAllClassnames;
	 ...
	 */
	public generate = (): ClassnamesTypes.TAllClassnames => {
		return this._generatedRegularClassnames;
	};

	private layout = (): ClassnamesTypes.Layout => {
		return {
	...
	private grouping = (): ClassnamesTypes.Grouping => {
		return {
			group: ["group"],
			peer: ["peer"],
		};
	...
	private backgrounds = (): ClassnamesTypes.Backgrounds => {
		return {
	...
	private borders = (): ClassnamesTypes.Borders => {
		return {
	...
	private tables = (): ClassnamesTypes.Tables => {
		return {
	...
	private effects = (): ClassnamesTypes.Effects => {
		return {
	...
	private transitionsAndAnimations = (): ClassnamesTypes.TransitionsAndAnimations => {
		return {
			...nonConfigurableClassNames.transitionsAndAnimations,
			transitionProperty: Object.keys(this._theme.transitionProperty).map(property => "transition-" + property),
			transitionDuration: Object.keys(this._theme.transitionDuration).map(value => "duration-" + value),
	...
	private transforms = (): ClassnamesTypes.Transforms => {
		return {
			...nonConfigurableClassNames.transforms,
			scale: ["", "x-", "y-"].flatMap(x => Object.keys(this._theme.scale).map(value => "scale-" + x + value)),
			...
			// by default and theme.translate overrides this behavior.
			translate: ["x", "y"].flatMap(side => {
				return Object.keys(_.isEmpty(this._theme.translate) ? this._theme.spacing : this._theme.translate).map(
					value =>
						value.startsWith("-") ? `-translate-${side}-${value.slice(1)}` : `translate-${side}-${value}`,
				);
			}),
	...
	private interactivity = (): ClassnamesTypes.Interactivity => {
	...
	private SVG = (): ClassnamesTypes.SVG => {
		return {
	...
	private accessibility = (): ClassnamesTypes.Accessibility => {
		return {
			...nonConfigurableClassNames.accessibility,
		};
	};

	private filters = (): ClassnamesTypes.Filters => {
		return {
			...
			backdropBrightness: Object.keys(this._theme.backdropBrightness).map(x => "backdrop-brightness-" + x),
			backdropContrast: Object.keys(this._theme.backdropContrast).map(x => "backdrop-contrast-" + x),
			backdropGrayscale: Object.keys(this._theme.backdropGrayscale).map(x => "backdrop-grayscale-" + x),
			...
			backdropSaturate: Object.keys(this._theme.backdropSaturate).map(x => "backdrop-saturate-" + x),
			backdropSepia: Object.keys(this._theme.backdropSepia).map(x => "backdrop-sepia-" + x),
		};
	};

	private flexBox = (): ClassnamesTypes.FlexBox => {
		return {
	...
	private grid = (): ClassnamesTypes.Grid => {
		return {
			...nonConfigurableClassNames.grid,
			gridTemplateColumns: Object.keys(this._theme.gridTemplateColumns).map(key => `grid-cols-${key}`),
	...
	private spacing = (): ClassnamesTypes.Spacing => {
		const sides = ["", "y", "x", "t", "r", "b", "l"];
		return {
			padding: sides.flatMap(side => {
				return Object.keys(_.isEmpty(this._theme.padding) ? this._theme.spacing : this._theme.padding).map(
					value => (value.startsWith("-") ? `-p${side}-${value.slice(1)}` : `p${side}-${value}`),
				);
			}),
			margin: sides.flatMap(side => {
				return Object.keys(_.isEmpty(this._theme.margin) ? this._theme.spacing : this._theme.margin).map(
					value => (value.startsWith("-") ? `-m${side}-${value.slice(1)}` : `m${side}-${value}`),
				);
			}),
	...
	private sizing = (): ClassnamesTypes.Sizing => {
		// prettier-ignore
      ...
      '5/12', '6/12', '7/12', '8/12', '9/12', '10/12', '11/12'];
		...
		return {
			...
			// and theme.width overrides this default behavior.
			// prettier-ignore
	...
	private typography = (): ClassnamesTypes.Typography => {
		return {
			...
			textDecorationThickness: Object.keys(this._theme.textDecorationThickness).map(x => "decoration-" + x),
			textUnderlineOffset: Object.keys(this._theme.textUnderlineOffset).map(x => "underline-offset-" + x),
		};
	};
	...
	private pseudoClasses = (): string[] => {
		// Initialize a pseudoClasses array with base values.
		...
		for (const regularClassGroupKey of regularClassGroupKeys) {
			Object.keys(this._generatedRegularClassnames).map(key => {
				...
				if (
					_.has(
						this._generatedRegularClassnames[key as keyof ClassnamesTypes.TAllClassnames],
	...
	private getGeneratedClassesWithOpacities = (): ClassesWithOpacities => {
      ...
      | 'borderOpacity' | 'placeholderOpacity' | 'ringOpacity'
		...
		function generateOpacities(
			...
			const result = extendedThemeOpacities ? { ...themeOpacities, ...extendedThemeOpacities } : themeOpacities;

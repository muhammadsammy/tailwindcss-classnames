...
export class GeneratedFileWriter {
	...
	private generateFileContent = async (): Promise<string> => {
		...
		const fileContentTemplate = new FileContentGenerator(generatedClassnames, configParser).generateFileContent();
		...
		if (!!this._outputFilename && !!this._customClassesFilename) {
			...
			customClassesImportPath =
				customClassesImportPath[0] === "." ? customClassesImportPath : `./${customClassesImportPath}`;
		}
			...
			return fs
				...
				.then(data => {
					if (!data.toString().includes("export default")) {
						this.printCliMessage("error", "The type having the custom classes must be a default export");
					}
				})
				.catch(error => {
